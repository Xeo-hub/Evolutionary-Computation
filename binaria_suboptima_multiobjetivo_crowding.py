...
def calcular_crowding_distance(poblacion):
    # Inicializa la lista de Crowding Distances para todas las soluciones en la población.
    crowding_distances = [0] * len(poblacion)
    num_objetivos = 2  # En este caso hay dos objetivos, distancia y costes

    # Itera sobre cada uno de los objetivos.
    for i in range(num_objetivos):
        # Ordena la población según el valor del objetivo actual.
        sorted_population = sorted(poblacion, key=lambda x: evaluar_individuo(x)[i])

        # Establece las Crowding Distances de los puntos extremos como infinito para mantenerlos en la población.
        crowding_distances[0] = crowding_distances[-1] = float('inf')

        # Encuentra el valor mínimo y máximo del objetivo actual.
        min_obj = evaluar_individuo(sorted_population[0])[i]
        max_obj = evaluar_individuo(sorted_population[-1])[i]

        # Si el mínimo es igual al máximo, no hace falta calcular Crowding Distances para este objetivo.
        if min_obj == max_obj:
            continue

        # Calcula la Crowding Distance para los puntos intermedios.
        for j in range(1, len(poblacion) - 1):
            crowding_distances[j] += (evaluar_individuo(sorted_population[j + 1])[i] - evaluar_individuo(sorted_population[j - 1])[i]) / (max_obj - min_obj)

    #  Devuelve la lista de Crowding Distances calculada para cada solución en la población.
    return crowding_distances

def ordenar_poblacion_crowding(poblacion, pareto_frente, crowding_distances):
    poblacion_nueva = []  # Crear una nueva población ordenada

    # Iterar a través de los frentes de Pareto en orden
    for frente in pareto_frente:
        # Igual esto se puede optimizar en vez de usar el .index
        calidad_diversidad = [(crowding_distances[poblacion.index(i)], i) for i in frente]
        calidad_diversidad.sort(key=lambda x: x[0], reverse=True)  # Ordenar por Crowding Distance en orden descendente
        # Obtener el índice original de las soluciones clasificadas en este frente
        poblacion_nueva.extend(solucion[1] for solucion in calidad_diversidad)
...
try:
    for i in range(num_generaciones):
        ...
        for j in range(numero_islas):
            ...
            # APLICAMOS CROWDING DISTANCE PARA SELECCIONAR A LOS MEJORES INDIVIDUOS

            # Calcula las distancias de crowding para los individuos
            crowding_distances = calcular_crowding_distance(poblaciones_islas[j])
            # Calculamos la media de crowding distance y la desviación estándar, que nos servirá para determinar la diversidad de la población
            media_crowding_distance =  np.mean(crowding_distances[1:-1])
            std_crowding_distance = np.std(crowding_distances[1:-1])
        
            # Encuentra los frentes de Pareto
            frentes_pareto = encontrar_frentes_de_pareto(poblaciones_islas[j])

            # Sacamamos la nueva población ya ordenada mediante la combinación de criterios (crowding distance y frente de pareto)
            poblacion_ordenada = ordenar_poblacion_crowding(poblaciones_islas[j], frentes_pareto, crowding_distances)

            # Selecciona los mejores individuos según el ranking
            poblaciones_islas[j] = poblacion_ordenada[:tamaño_generacion+1]
            ...
            print("Valor medio de crowding distance en población: ", media_crowding_distance)
            print("Desviación estándar de crowding distance en población:", std_crowding_distance)
        ...
        for i in mejores_fitness: 
            if i not in mejor_fitness_general_generacion:
                mejor_fitness_general_generacion.append(i) 
    ...
    if mejor_individuo_actual:
        ...
        print("Número de mejores individuos: ", len(mejor_individuo_actual))
        print(mejor_individuo_actual)
        print("Número de mejores fitnesss asociados: ", len(mejores_fitness))
